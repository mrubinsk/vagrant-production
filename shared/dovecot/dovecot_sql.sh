...
echo 'TODO: automatically enable ssl'
if which dovecot > /dev/null; then
    ...
    echo -e "connect = host=127.0.0.1 dbname=mail user=mail password=$MYSQLMAILPASSWORD" | sudo tee -a /etc/dovecot/dovecot-sql.conf.ext
    ...
    echo -e 'mail_location = maildir:/var/vmail/%u' | sudo tee -a /etc/dovecot/conf.d/10-mail.conf
    ...
    # These certs will be created via LE
    # if [[ ! -f /etc/dovecot/conf.d/10-ssl.conf.orig ]]; then
    #     mv /etc/dovecot/conf.d/10-ssl.conf /etc/dovecot/conf.d/10-ssl.conf.orig
    # fi
    echo -e 'ssl=yes' | sudo tee -a /etc/dovecot/conf.d/10-ssl.conf
    echo -e 'ssl_cert = </etc/letsencrypt/live/tarn.theupstairsroom.com/fullchain.pem' | sudo tee -a /etc/dovecot/conf.d/10-ssl.conf
    echo -e 'ssl_key = </etc/letsencrypt/live/tarn.theupstairsroom.com/privkey.pem' | sudo tee -a /etc/dovecot/conf.d/10-ssl.conf
