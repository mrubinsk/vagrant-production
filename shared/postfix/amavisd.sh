...
sed -i '/^\s*AllowSupplementaryGroups/s/^/# /g' /etc/clamav/clamd.conf
echo -e 'AllowSupplementaryGroups true' | sudo tee -a /etc/clamav/clamd.conf
echo -e "\$myhostname = '$DOMAIN';" | sudo tee -a /etc/amavis/conf.d/05-node_id
